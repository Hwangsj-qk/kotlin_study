...
//    age = "홍길동"     // 추론된 Type이 Int이기 때문에 숫자만 대입 가능
    ...
    // 명시적 타입 표시
    // java와의 차이점 : "타입이 변수명 : 뒤에 선언"
    val byte : Byte = 127
    val int : Int = 1272343
    val Long : Long = 12723433455L
    ...
    // 코드에서는 평번한 클래스로 보이지만 실행시점에 Primitive Type 으로 실행
    // Boxing, Unboxing 고려 없이 코틀린이 알아서 처리

    val longVal = 100L      // L을 붙이면 따로 Long이라고 표시해주지 않아도 Long 타입으로 추론
    ...
    // 1. Any : 모든 클래스의 최상위 루트 클래스 (자바의 Object 해당)
   ...
   // 모든 코틀린 객체를 담을 수 있고, 모든 객체는 Any를 상속받는다.
    ...
    // 2. Unit 타입 : 함수가 아무 값을 리턴하지 않을 때 (자바의 void에 해당)
