...
// null값이 포함될 수 있는 속성이나 함수는 Safety Call을 통해 호출
// null값이 감지될 경우 어떻게 할지 선언

fun main() {
    // non-nullable 타입 (null 불가능 타입)
    var name: String = "홍길동"
    ...
    println(address?.length)        // null이 아닌 경우: 값을 호출(2)

    address = null
    println(address?.length)    // null인 경우 : null 출력
    // 안전 호출 연산자는 뒷부분을 수행하지 않음
